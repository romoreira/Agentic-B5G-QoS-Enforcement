#!/usr/bin/env python3
# E4b agentic hysteresis runner.
#
# Differences relative to the original E4 runner (run_agentic_realtime_run1.py):
#   1. candidates(): monotonic non-increasing under active app QER policing.
#      - In Phase B with app_drop > 0, only candidates with S5 <= current S5 are offered.
#      - In Phase C with app_drop > 0, only candidates with Silver <= current Silver are offered.
#      Gold is never touched. INITIAL_MBR is restored only in Phase D.
#   2. validate_decision(): action is derived from the chosen vector, not declared by the LLM.
#      - action = "keep" if mbr == current else "modify_mbr".
#      - Removes the failure mode "modify_mbr requires a changed vector" observed in E4.
#
# Everything else (snapshots, telemetry parsing, prompt, vLLM call, application via pfcpsim)
# is identical to the E4 baseline so that the comparison isolates the two structural changes.
...
# -----------------------------------------------------------------------------
# PATCH 1: monotonic-in-overload candidates.
# Under active app QER policing, only candidates that tighten the relevant tier
# are offered. Upward moves are only allowed in Phase D (recovery).
# -----------------------------------------------------------------------------
def candidates(phase, current, tel):
    ...
    if phase == "B":
        if app_drop > 0:
            ...
            options = [v for v in full if v[4] <= current[4]]
            return unique([current] + options), \
                "real app QER policing in bronze burst, monotonic non-increasing S5"
    ...
    if phase == "C":
        if app_drop > 0:
            ...
            options = [v for v in full
                       if v[2] <= current[2] and v[3] <= current[3]]
            return unique([current] + options), \
                "real app QER policing in silver flash crowd, monotonic non-increasing Silver, preserve Gold"
        return unique([current]), "no app QER policing observed"

    if phase == "D":
        # Recovery is still LLM mediated, but policy constrained.
        # If the current vector differs from the nominal SLA vector, the only
        # admissible recovery candidate is INITIAL_MBR.
        # The LLM still selects candidate_id and produces the audit reason.
        if current != INITIAL_MBR:
            return unique([INITIAL_MBR]), "recovery phase, only admissible candidate is the initial SLA vector"
        return unique([current]), "recovery phase, already at initial vector"
...
def validate_vector(mbr):
    if not isinstance(mbr, list) or len(mbr) != 5:
        raise ValueError("mbr_kbps must be a list of five values")
    if not all(isinstance(x, int) for x in mbr):
        raise ValueError("MBR values must be integers")
    if not all(x > 0 and x % 10000 == 0 for x in mbr):
        raise ValueError("MBR values must be positive multiples of 10000")
    if sum(mbr) > CAPACITY_LIMIT:
        raise ValueError(f"capacity exceeded, sum={sum(mbr)}")
    return mbr

def candidate_choices(cand):
    return [{"candidate_id": i, "mbr_kbps": v} for i, v in enumerate(cand)]

# -----------------------------------------------------------------------------
# PATCH 2b: select by candidate_id.
# The LLM no longer controls the final vector directly. It selects one candidate
# id, and the runner derives the MBR vector from the local candidate list.
# This prevents invented values such as 25000 and protects Gold in Phase C.
# -----------------------------------------------------------------------------
def validate_decision(parsed, current, cand):
    declared_action = parsed.get("action")
    selected_by = None
    ...
    if isinstance(cid, int):
        if cid < 0 or cid >= len(cand):
            raise ValueError(f"candidate_id out of range, candidate_id={cid}, candidates={candidate_choices(cand)}")
        mbr = list(cand[cid])
        selected_by = "candidate_id"
    elif "mbr_kbps" in parsed:
        # Backward compatible path for older model outputs.
        # It is accepted only when the vector exactly matches one candidate.
        mbr = validate_vector(parsed.get("mbr_kbps"))
        if mbr not in cand:
            raise ValueError(f"mbr not in candidates, mbr={mbr}, candidates={cand}")
        cid = cand.index(mbr)
        selected_by = "mbr_kbps_exact_match"
    else:
        raise ValueError("decision must include candidate_id or mbr_kbps")
    ...
    return {
        ...
        "expected_effect": str(parsed.get("expected_effect", ""))[:500],
    }

def call_llm(base_url, model, tel, cand, reason, timeout, max_tokens):
    prompt_tel = dict(tel)
    prompt_tel["candidate_choices"] = candidate_choices(cand)
    prompt_tel["candidate_reason"] = reason
    prompt_tel["selection_rule"] = (
        "Return only a candidate_id from candidate_choices. "
        "Do not create, edit, round, or infer any MBR vector. "
        "The controller will derive mbr_kbps from candidate_id."
    )
    payload = {
        "model": model,
        "messages": [
            {"role": "system", "content":
                "You are a policy constrained B5G UPF QoS controller. "
                "You must choose exactly one candidate_id from candidate_choices. "
                "Never invent a new MBR vector. Never edit a candidate vector. "
                "The only valid MBR vectors are those listed in candidate_choices. "
                "Use real telemetry from the last window. Protect Gold, then Silver, then Bronze. "
                "If phase is D and candidate_choices has one entry, select candidate_id 0 because it is the only admissible recovery action. ""If app_qer_meter_red_delta is zero and there are no datapath errors outside phase D, prefer the candidate whose vector equals current_mbr_kbps. "
                "If phase is B and app_qer_meter_red_delta is positive, choose a candidate that reduces only S5, but never below the lowest listed candidate. "
                "If phase is C and app_qer_meter_red_delta is positive, choose a candidate that preserves S1 and S2 Gold and adjusts only S3 and S4 Silver. "
                "If phase is D, prefer the candidate equal to the initial vector. "
                "Return only valid JSON, no markdown."},
            {"role": "user", "content":
                "Choose the next MBR candidate. Return exactly this JSON schema: "
                "{\"candidate_id\":int,"
                "\"action\":\"keep or modify_mbr\","
                "\"reason\":\"short reason based on real telemetry\","
                "\"expected_effect\":\"short expected effect\"}. "
                "Do not include mbr_kbps unless it is copied exactly from the selected candidate. "
                "Telemetry: " + json.dumps(prompt_tel)},
        ],
        "temperature": 0,
...
def fallback(current, reason):
    return {"action": "keep", "mbr_kbps": current,
            "declared_action": None,
            "reason": f"fallback keep current, {reason}",
            "expected_effect": "preserve current UPF configuration"}

def apply_mbr(mbr):
    mbr_csv = ",".join(map(str, mbr))
    cmd = ["sudo", "docker", "exec", "pfcpsim", "pfcpctl",
           "-s", "localhost:12345", "session", "modify",
           "--count", "5", "--baseID", "1",
...
def write_metadata(run_dir, args):
    ...
    data = {
        ...
        "run": args.run_name,
        "controller": "policy_constrained_llm_realtime_hysteresis",
        "patches": [
            "candidates: monotonic non-increasing under active app QER policing",
            "validate_decision: action derived from chosen candidate_id",
        ],
        "vllm_base_url": args.vllm_base_url,
        "vllm_model": args.vllm_model,
        "upf_ssh": args.upf_ssh,
        "window_sec": args.window_sec,
        "windows_per_phase": args.windows_per_phase,
        "initial_mbr_kbps": INITIAL_MBR,
        "capacity_limit_kbps": CAPACITY_LIMIT,
        "trex_profile": str(args.trex_profile),
        "scale": args.scale,
    }
    write(meta / "run_info.json", json.dumps(data, indent=2) + "\n")
    write(meta / "slice_config.csv",
          "slice,tier,teid,ue_ip,initial_cap_mbps\n"
...
def main():
    ...
    ap.add_argument("--root", type=Path,
                    default=Path.home() / "agentic_qos_results/campaigns/campaign_20260512_agentic_hysteresis_5slices")
    ap.add_argument("--experiment", default="e4b_agentic_hysteresis_realtime")
    ...
    ap.add_argument("--trex-profile", type=Path,
                    default=Path("/opt/trex/v3.08/automation/exp2/agentic_5slice_phase_profile.py"))
    ...
    with actions_path.open("w", newline="") as f:
        csv.writer(f).writerow(["timestamp_utc", "phase", "window_index",
                                "old_mbr_kbps", "new_mbr_kbps",
                                "reason", "tool_returncode"])
    ...
    for phase in PHASES:
        ...
        for w in range(1, args.windows_per_phase + 1):
            ...
            run_trex_window(args.trex_dir, args.trex_profile, phase,
                            args.window_sec, args.scale,
                            run_dir / "trex" / f"{label}_console.txt")
            ...
            fallback_used = False
            try:
                parsed, raw, latency, usage = call_llm(
                    args.vllm_base_url, args.vllm_model, tel, cand, reason,
                    args.llm_timeout, args.llm_max_tokens)
            ...
            if decision["mbr_kbps"] != current:
                ...
                if apply_result["returncode"] == 0:
                    ...
                    with actions_path.open("a", newline="") as f:
                        csv.writer(f).writerow([now(), phase, w,
                                                ",".join(map(str, old)),
                                                ",".join(map(str, current)),
                                                decision.get("reason", ""), 0])
                else:
            ...
            rec = {
                ...
                "old_mbr_kbps": old,
                "new_mbr_kbps": current,
                "apply_result": apply_result,
            }
            append_jsonl(decisions_path, rec)
            log(f"Decision phase={phase} window={w} action={decision['action']} "
                f"old={old} new={current} fallback={fallback_used}")
    ...
    summary = {
        ...
        "total_fallbacks": totals["fallbacks"],
        "total_tool_failures": totals["tool_failures"],
        "final_mbr_kbps": current,
        "window_sec": args.window_sec,
        "windows_per_phase": args.windows_per_phase,
        "phases": PHASES,
    }
    write(run_dir / "controller/run_summary.json", json.dumps(summary, indent=2) + "\n")
    write(run_dir / "logs/pfcpsim_logs_tail.txt",
          sh("sudo docker logs pfcpsim --tail 200", timeout=30).stdout)
